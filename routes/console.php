...
if (app()->environment("production") && !getenv("LARAVEL_MIGRATE_ORIGINAL")) {
	...
	Artisan::command("migrate:fresh {--database=} {--path=*} {--realpath} {--schema-path=} {--seed} {--seeder=} {--step} {--drop-views} {--drop-types} {--force} {--p=}", function () {
		...
		foreach ($this->options() as $option => $value) {
			...
			if (!in_array($option, ['command', 'p', 'help', 'quiet', 'verbose', 'version', 'ansi', 'no-ansi', 'no-interaction', 'env'])) {
				if ($value === true) {
					$command .= " --{$option}";
	...
	Artisan::command("migrate {--database=} {--force} {--path=*} {--realpath} {--pretend} {--seed} {--seeder=} {--step} {--isolated}", function () {
		...
		foreach ($this->options() as $option => $value) {
			...
			if (!in_array($option, ['command', 'help', 'quiet', 'verbose', 'version', 'ansi', 'no-ansi', 'no-interaction', 'env'])) {
				if ($value === true) {
					$command .= " --{$option}";
				} elseif (is_array($value) && !empty($value)) {
					foreach ($value as $arrValue) {
