...
$(window).scroll(function () {
	...
	if (scroll >= 200) {
		...
		$("#imgLogo")
			.attr("src", "/workspace/static/images/logos/logotestataRENUDO-black.svg")
			.css({ transition: "height .375s ease-out" });
	} else {
		...
		$("#imgLogo")
			.attr("src", "/workspace/static/images/logos/logotestataRENUDO.svg")
			.css({ transition: "height .375s ease-out" });
	}
});
...
$(document).ready(function () {
	...
	$(".menu_navbar:not(.d-none) .btn_toggler_open").on("click", function () {
		...
		let navBgColorRight = window
			.getComputedStyle(document.documentElement, null)
			.getPropertyValue("--nav_bg_color_right");
		let navBgColorBottom = window
			.getComputedStyle(document.documentElement, null)
			.getPropertyValue("--nav_bg_color_bottom");
		let navBgColorLeft = window.getComputedStyle(document.documentElement, null).getPropertyValue("--nav_bg_color_left");

		let navBgPositionTop = window
			.getComputedStyle(document.documentElement, null)
			.getPropertyValue("--nav_bg_position_top");
		let navBgPositionRight = window
			.getComputedStyle(document.documentElement, null)
			.getPropertyValue("--nav_bg_position_right");
		let navBgPositionBottom = window
			.getComputedStyle(document.documentElement, null)
			.getPropertyValue("--nav_bg_position_bottom");
		let navBgPositionLeft = window
			.getComputedStyle(document.documentElement, null)
			.getPropertyValue("--nav_bg_position_left");

		if ($(this).attr("aria-expanded") === "false") {
			// Click when menu is open
			...
			$(".nav_grow_top .nav_ghost_bg, .nav_slide_top .nav_ghost_bg").css({
				"background": navBgColorTop,
				"background-size": "300% 300%",
				"transition": "background-position .375s ease-out",
			});
			$(".nav_grow_bottom .nav_ghost_bg, .nav_slide_bottom .nav_ghost_bg").css({
				"background": navBgColorBottom,
				"background-size": "300% 300%",
				"transition": "background-position .375s ease-out",
			});

			if ($(".coll_ghost_bg").length) {
				$(".nav_grow_right .nav_ghost_bg, .nav_slide_right .nav_ghost_bg").css({
					"background": navBgColorLeft,
					"background-size": "300% 300%",
					"transition": "background-position .375s ease-out",
				});
				$(".nav_grow_left .nav_ghost_bg, .nav_slide_left .nav_ghost_bg").css({
					"background": navBgColorRight,
					"background-size": "300% 300%",
					"transition": "background-position .375s ease-out",
				});
			} else {
			...
			$(".scrollbar_spacing").attr({
				style: "padding-right: " + scrollbarWidth + "px; transition: border-color .375s ease-out;",
			});

			if ($(".menu_steps, .btn_add_soluzione").attr("aria-expanded") === "false") {
				// Click when menu closes
				...
				$(".coll_ghost_bg").css({
					"z-index": "-2",
					"max-width": "calc(" + collGhostWidth + " + " + scrollbarWidth + "px)",
					"background-position": navBgPositionLeft,
					"transition": "background-position .375s ease-out",
				});
			...
			$(".nav_grow_top .nav_ghost_bg, .nav_slide_top .nav_ghost_bg").css({
				"background-position": navBgPositionTop,
				"transition": "background-position .375s ease-out",
			});
			$(".nav_grow_bottom .nav_ghost_bg, .nav_slide_bottom .nav_ghost_bg").css({
				"background-position": navBgPositionBottom,
				"transition": "background-position .375s ease-out",
			});
			...
			if ($(".coll_ghost_bg").length) {
				...
				$(".nav_grow_left .nav_ghost_bg, .nav_slide_left .nav_ghost_bg").css({
					"background-position": navBgPositionRight,
					"transition": "background-position .375s ease-out",
				});
			} else {
				$(".nav_grow_right .nav_ghost_bg, .nav_slide_right .nav_ghost_bg").css({
	...
	$(".menu_steps, .menu_options .btn_toggler, .btn_add_soluzione").on("click", function () {
		// Click on cart
		const scrollbarWidth = Math.abs(window.innerWidth - document.documentElement.clientWidth);
		...
		if ($(".menu_steps, .btn_add_soluzione").attr("aria-expanded") === "false") {
			...
			if ($(".menu_navbar:not(.d-none) .btn_toggler_open").attr("aria-expanded") === "false") {
				...
				$(".nav_ghost_bg").css({
					"z-index": "-1",
					"transition": "background-position .375s ease-out, z-index .375s ease-out",
				});
				$(".coll_ghost_bg").css({
					"z-index": "-2",
					"max-width": "calc(" + collGhostWidth + " + " + scrollbarWidth + "px)",
	...
	$("[data-bs-toggle='modal']").on("click", function () {
		...
		if (
			$(".menu_navbar:not(.d-none) .btn_toggler_open, .menu_steps, .btn_add_soluzione").attr("aria-expanded") ===
			"false"
		) {
			// if ($(".menu_navbar:not(.d-none) .btn_toggler_open").attr("aria-expanded") === "false" &&
	...
	$(".modal").on("hidden.bs.modal", function () {
		...
		if (
			$(".menu_navbar:not(.d-none) .btn_toggler_open").attr("aria-expanded") === "false" &&
			$(".menu_steps").attr("aria-expanded") === "false" &&
			$(".btn_add_soluzione").attr("aria-expanded") === "false"
		) {
...
$("body").on("click", ".add_voto", function (e) {
	...
	$.post(
		"https://www.associazionedimorestoricheitaliane.it/action-candidati/",
		{
			"fields[member]": member,
			"fields[votazione]": vota,
			"fields[data]": strDateTime,
			"action[add-votante]": "submit",
		},
		function (data) {
			$("#addvoto").submit();
		}
...
$(document).ready(function () {
	...
	$("body").on("click", ".serializeandhtml", function () {
		...
		$.when(
			ajaxcall($("." + formref), $(this).attr("data-url"), $(this).attr("data-msg1"), $(this).attr("data-msg2"))
		).done(function (result) {
			if (result) {
...
function addprodtobasket(elem) {
	...
	$.post(
		$("#checkship").attr("data-url"),
		{
			"fields[qty]": refqty,
			"fields[price]": refprice,
			"fields[order]": idorder,
			"fields[product]": refprod,
			"fields[specific]": refspec,
			"action[add-order-detail]": "submit",
		},
		function (data) {
...
function htmltodiv(elem, urldest) {
	var url = urldest;
	var loading =
		'<img src="https://www.associazionedimorestoricheitaliane.it/workspace/static/bootstrap/images/originaloading.gif" style="margin: 0% 33%;width: 100px;"/>';
	$.ajax({
		url: url,
		beforeSend: function () {
			elem.html(loading);
