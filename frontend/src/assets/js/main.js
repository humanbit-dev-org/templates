...
function addDisponibility(id) {
	$(document).ready(function () {
		...
		for (i = 0; i < solution.length; i++) {
			...
			$(form)
				.find('input[name="' + input_solution + '"]')
				.val(solution_id);
			$(form)
				.find('input[name="' + input_amount + '"]')
				.val(solution_amount);
			$(form)
				.find('input[name="' + input_size + '"]')
				.val(solution_size);
			$.ajax({
				type: "POST",
				data: $(form).serialize(),
	...
	if ($(class_container).find(class_content).length > 0) {
		...
		if ($(class_container).find(class_content).find(".change_size").length > 0) {
			...
			if (check === 1) {
				...
				countTriggerStyle(
					$(class_container).find(class_content).length,
...
$(document).ready(function () {
	...
	$("body").on("change", ".change_size", function () {
		...
		$(form)
			.find('input[name="' + input_notes + '"]')
			.val(val);
		$(form)
			.find('input[name="' + input_order + '"]')
			.val(order_value);
		$.ajax({
			type: "POST",
			data: $(form).serialize(),
	...
	$("body").on("change", ".change_amount", function () {
		...
		$(form)
			...
			.val(amount_value);
		$(form)
			.find('input[name="' + input_price + '"]')
			.val(price);
		$(form)
			.find('input[name="' + input_price_tot + '"]')
			.val(price_tot.toFixed(2));
		$.ajax({
			type: "POST",
			data: $(form).serialize(),
			url: url,
			success: function (data) {
				...
				if (price_checkout != undefined) {
					...
					if (orderPrice >= minBasket) {
						...
						$("#add_order")
							.find('input[name="fields[final-price]"]')
							.val(Number(final_price_spedition).toFixed(2));
						$("#add_order").find('input[name="fields[order-cost]"]').val(Number(orderPrice).toFixed(2));
						$("#add_order").find('input[name="fields[shipping-cost]"]').val(spedPrice);
					}
				}
					...
					$.ajax({
						...
						success: function (data) {
							...
							if (orderPrice >= minBasket) {
								...
								$("#add_order")
									.find('input[name="fields[final-price]"]')
									.val(Number(final_price).toFixed(2));
								$("#add_order").find('input[name="fields[order-cost]"]').val(Number(orderPrice).toFixed(2));
								$("#add_order").find('input[name="fields[shipping-cost]"]').val("0");
							} else {
								$(".delivery_price").attr("style", "");
	...
	$("body").on("click", ".btn_add_soluzione", function (e) {
		...
		$(form)
			.find('input[name="' + input_soluzione + '"]')
			.val(soluzione_id);
		$(form)
			.find('input[name="' + input_price_tot + '"]')
			.val(price_tot.toFixed(2));
