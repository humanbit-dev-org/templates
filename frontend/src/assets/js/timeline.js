...
$(document).ready(function () {
	...
	$(".chat_text_input").on("input", function () {
		...
		var placeholders = [
			"Meglio 100 piegamenti o un task di ADSI?",
			"Ho delle idee per migliorare il template!",
			"Mezza giornata?",
			"Ricordati della riunione alle 12.00, mi raccomando...",
			"Mi mandi i codici colori?",
			"Oggi quale cliente insultiamo?",
		];
		if ($(this_message).val() === "") {
		...
		if (popup_start === 1 && popup_end === 0) {
			...
			if ($(popup_action_start_first).length > 0) {
				$(popup_action_start_first)
					.removeClass("popup_action_start_first")
					.addClass("popup_action_start_first_active");
			}
			if ($(popup_action_start_second).length > 0) {
				$(popup_action_start_second)
					.removeClass("popup_action_start_second")
					.addClass("popup_action_start_second_active");
			}
			popup_start = 1;
